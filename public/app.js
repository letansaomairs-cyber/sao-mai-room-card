...
// ===== QUẢN LÝ VÉ BƠI THÁNG =====
const POOL_API='/api/pool-passes';let poolItems=[];
function poolPin(){return $('#poolPin')?.value||''}
function poolDate(v){if(!v)return '-';const [y,m,d]=v.split('-');return `${d}/${m}/${y}`}
function poolIso(v){v=String(v||'').trim();if(/^\d{4}-\d{2}-\d{2}$/.test(v))return v;const m=v.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})$/);if(!m)return '';const d=m[1].padStart(2,'0'),mo=m[2].padStart(2,'0'),y=m[3];return `${y}-${mo}-${d}`}
function poolToday(){const d=new Date();d.setHours(0,0,0,0);return d}
function initPoolDates(){if(typeof flatpickr!=='function')return;const opts={dateFormat:'Y-m-d',altInput:true,altFormat:'d/m/Y',allowInput:true,locale:{firstDayOfWeek:1}};if($('#poolStart')&&!$('#poolStart')._flatpickr)flatpickr('#poolStart',{...opts,defaultDate:new Date()});if($('#poolEnd')&&!$('#poolEnd')._flatpickr){const d=new Date();d.setMonth(d.getMonth()+1);flatpickr('#poolEnd',{...opts,defaultDate:d})}}
function poolState(x){const p=String(x.end_date||'').split('-').map(Number),n=new Date(),end=Date.UTC(p[0],p[1]-1,p[2]),today=Date.UTC(n.getFullYear(),n.getMonth(),n.getDate()),days=Math.round((end-today)/86400000);if(days<0)return {key:'expired',label:'Đã hết hạn',days};if(days<=3)return {key:'soon',label:'Sắp hết hạn',days};return {key:'valid',label:'Còn hạn',days}}
function poolRemain(st){if(st.key==='expired')return `Quá hạn ${Math.abs(st.days)} ngày`;if(st.days===0)return 'Hết hạn hôm nay';return `${st.days} ngày`}
async function loadPool(){try{poolItems=await APIX(POOL_API,{headers:{'x-admin-pin':poolPin()}});$('#poolArea').classList.remove('hidden');renderPool()}catch(e){alert(e.message)}}
function renderPool(){const q=($('#poolSearch')?.value||'').trim().toLowerCase(),type=$('#poolTypeFilter')?.value||'',status=$('#poolStatusFilter')?.value||'';let valid=0,soon=0,expired=0;poolItems.forEach(x=>{const s=poolState(x);if(s.key==='valid')valid++;else if(s.key==='soon')soon++;else expired++});$('#poolValid').textContent=valid;$('#poolSoon').textContent=soon;$('#poolExpired').textContent=expired;$('#poolTotal').textContent=poolItems.length;const rows=poolItems.filter(x=>{const s=poolState(x);return(!q||String(x.guest_name).toLowerCase().includes(q))&&(!type||x.pass_type===type)&&(!status||s.key===status)});$('#poolRows').innerHTML=rows.length?rows.map(x=>{const s=poolState(x);return `<tr><td><b>${E(x.guest_name)}</b>${x.notes?`<br><small>${E(x.notes)}</small>`:''}</td><td>${x.pass_type==='large'?'Vé tháng lớn':'Vé tháng nhỏ'}</td><td>${poolDate(x.start_date)}</td><td>${poolDate(x.end_date)}</td><td class="pool-days">${poolRemain(s)}</td><td><span class="pool-status ${s.key}">${s.label}</span></td><td><div class="pool-actions"><button class="secondary" onclick="POOL_RENEW(${x.id})">Gia hạn</button><button class="secondary" onclick="POOL_EDIT(${x.id})">Sửa</button><button class="danger" onclick="POOL_DELETE(${x.id})">Xóa</button></div></td></tr>`}).join(''):'<tr><td colspan="7" style="text-align:center;color:var(--muted)">Chưa có vé phù hợp</td></tr>'}
$('#openPool').onclick=loadPool;$('#poolRefresh').onclick=loadPool;$('#poolSearch').oninput=renderPool;$('#poolTypeFilter').onchange=renderPool;$('#poolStatusFilter').onchange=renderPool;
$('#poolForm').onsubmit=async e=>{e.preventDefault();try{await APIX(POOL_API,{method:'POST',headers:{'content-type':'application/json','x-admin-pin':poolPin()},body:JSON.stringify({guest_name:$('#poolGuest').value.trim(),pass_type:$('#poolType').value,start_date:$('#poolStart').value,end_date:$('#poolEnd').value,notes:$('#poolNote').value.trim()})});e.target.reset();await loadPool()}catch(err){alert(err.message)}};
window.POOL_DELETE=async id=>{if(!confirm('Xóa vé bơi tháng này?'))return;try{await APIX(`${POOL_API}/${id}`,{method:'DELETE',headers:{'x-admin-pin':poolPin()}});await loadPool()}catch(e){alert(e.message)}};
window.POOL_EDIT=async id=>{const x=poolItems.find(v=>Number(v.id)===Number(id));if(!x)return;const g=prompt('Tên khách',x.guest_name);if(g===null)return;const type=prompt('Loại vé: large = lớn, small = nhỏ',x.pass_type);if(type===null)return;const startText=prompt('Từ ngày (DD/MM/YYYY)',poolDate(x.start_date));if(startText===null)return;const endText=prompt('Đến ngày (DD/MM/YYYY)',poolDate(x.end_date));if(endText===null)return;const start=poolIso(startText),end=poolIso(endText);if(!start||!end)return alert('Ngày không hợp lệ. Vui lòng nhập theo DD/MM/YYYY.');const notes=prompt('Ghi chú',x.notes||'');if(notes===null)return;try{await APIX(`${POOL_API}/${id}`,{method:'PATCH',headers:{'content-type':'application/json','x-admin-pin':poolPin()},body:JSON.stringify({guest_name:g.trim(),pass_type:type.trim(),start_date:start,end_date:end,notes:notes.trim()})});await loadPool()}catch(e){alert(e.message)}};
window.POOL_RENEW=async id=>{const x=poolItems.find(v=>Number(v.id)===Number(id));if(!x)return;const endText=prompt(`Gia hạn vé của ${x.guest_name} đến ngày (DD/MM/YYYY)`,poolDate(x.end_date));if(endText===null||!endText.trim())return;const end=poolIso(endText);if(!end)return alert('Ngày không hợp lệ. Vui lòng nhập theo DD/MM/YYYY.');try{await APIX(`${POOL_API}/${id}`,{method:'PATCH',headers:{'content-type':'application/json','x-admin-pin':poolPin()},body:JSON.stringify({...x,end_date:end})});await loadPool()}catch(e){alert(e.message)}};
