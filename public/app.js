// ROOM CARD MANAGER - STATUS LOGIC V3 - 2026-08-07
...
function EFFECTIVE_STATUS(x){
  ...
  // Còn ít nhất 1 thẻ chưa trả/chưa mất -> vẫn là ĐANG SỬ DỤNG.
  if(remaining > 0){
    return 'active';
  }

  // Hết thẻ còn giữ và có thẻ mất.
  if(lost > 0){
...
function RA(){
  ...
  // THỐNG KÊ THEO TRẠNG THÁI THỰC TẾ, KHÔNG THEO status CŨ TRONG DB.
...
function AB(x){
  ...
  // Nếu vẫn còn thẻ khách đang giữ thì luôn cho trả/báo mất tiếp,
  // kể cả phiếu đang có status "paid".
  if(remaining > 0){
