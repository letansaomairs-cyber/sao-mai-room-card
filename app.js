...
function poolState(x){const e=new Date(x.end_date+'T23:59:59'),t=poolToday(),days=Math.ceil((e-t)/86400000);if(days<0)return {key:'expired',label:'Đã hết hạn',days};if(days<=7)return {key:'soon',label:'Sắp hết hạn',days};return {key:'valid',label:'Còn hạn',days}}
...
window.POOL_EDIT=async id=>{const x=poolItems.find(v=>Number(v.id)===Number(id));if(!x)return;const g=prompt('Tên khách',x.guest_name);if(g===null)return;const type=prompt('Loại vé: large = lớn, small = nhỏ',x.pass_type);if(type===null)return;const start=prompt('Từ ngày (YYYY-MM-DD)',x.start_date);if(start===null)return;const end=prompt('Đến ngày (YYYY-MM-DD)',x.end_date);if(end===null)return;const notes=prompt('Ghi chú',x.notes||'');if(notes===null)return;try{await APIX(`${POOL_API}/${id}`,{method:'PATCH',headers:{'content-type':'application/json','x-admin-pin':poolPin()},body:JSON.stringify({guest_name:g.trim(),pass_type:type.trim(),start_date:start.trim(),end_date:end.trim(),notes:notes.trim()})});await loadPool()}catch(e){alert(e.message)}};
window.POOL_RENEW=async id=>{const x=poolItems.find(v=>Number(v.id)===Number(id));if(!x)return;const end=prompt(`Gia hạn vé của ${x.guest_name} đến ngày (YYYY-MM-DD)`,x.end_date);if(end===null||!end.trim())return;try{await APIX(`${POOL_API}/${id}`,{method:'PATCH',headers:{'content-type':'application/json','x-admin-pin':poolPin()},body:JSON.stringify({...x,end_date:end.trim()})});await loadPool()}catch(e){alert(e.message)}};
