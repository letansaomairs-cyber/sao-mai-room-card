...
function ES(x){
  ...
  // Còn bất kỳ thẻ nào khách đang giữ -> trạng thái chính là Đang sử dụng
  if(remaining > 0) return 'active';

  // Không còn thẻ nào, nhưng có thẻ mất
  if(lost > 0){
    return x.status === 'paid' ? 'paid' : 'lost';
  }

  // Không mất thẻ và đã trả đủ
...
function RA(){
  ...
  const effectiveItems = adminItems.map(x => ({
    ...x,
    _effectiveStatus: ES(x)
  }));

  const a = effectiveItems.filter(x =>
    (!f || x._effectiveStatus === f) &&
    JSON.stringify(x).toLowerCase().includes(q)
  );

  $('#countActive').textContent =
    effectiveItems.filter(x => x._effectiveStatus === 'active').length;

  $('#countReturned').textContent =
    effectiveItems.filter(x => x._effectiveStatus === 'returned').length;

  $('#countLost').textContent =
    effectiveItems.filter(x =>
  ...
  $('#totalCompensation').textContent = M(
    adminItems.reduce(
      (s, x) => s + Number(x.compensation_amount || 0),
      0
    )
  );
  ...
  $('#adminRows').innerHTML = a.map(x => {
    ...
    // Nếu còn thẻ nhưng trước đó đã có mất/thu phí, vẫn hiển thị trạng thái chính là Đang sử dụng
    if(status === 'active' && lost > 0){
      if(x.status === 'paid'){
        statusExtra =
          `<br><small>Đã thu phí ${lost} thẻ · ${M(x.compensation_amount || 0)}</small>`;
      }else{
        statusExtra =
          `<br><small>Đã báo mất ${lost} thẻ · ${M(x.compensation_amount || 0)}</small>`;
      }
    }else if(
      (status === 'lost' || status === 'paid') &&
      Number(x.compensation_amount || 0) > 0
    ){
      statusExtra =
        `<br><small>${M(x.compensation_amount || 0)}</small>`;
    }
...
function AB(x){
  ...
  // Còn thẻ khách đang giữ: luôn cho phép trả tiếp hoặc báo mất tiếp
  if(remaining > 0){
    let buttons = '';
    ...
    // Chỉ cho hủy khi chưa có thẻ mất
  ...
  // Không còn thẻ khách giữ, có mất nhưng chưa thu phí
  if(x.status === 'lost' && lost > 0){
    return `
      <button
        class="primary small-btn"
